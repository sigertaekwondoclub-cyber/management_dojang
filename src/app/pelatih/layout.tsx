...
export default function PelatihLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background font-sans">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-dark border-t-secondary rounded-full animate-spin" />
          <span className="font-bold text-dark/60">Memuat Dashboard Pelatih...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row relative">
      ...
      <aside className="hidden md:flex w-64 bg-white border-r-4 border-dark p-6 flex-col">
            ...
            return (
              <Link key={item.path} href={item.path} prefetch={true}>
                <div className={`px-4 py-2.5 rounded-xl border-2 font-bold font-sans transition-all duration-150 cursor-pointer text-sm ${
                  isActive
                  ? 'bg-secondary border-dark shadow-brutal translate-x-[-2px] translate-y-[-2px]'
        ...
        <Button variant="accent" onClick={handleLogout} className="mt-8">Logout</Button>
      </aside>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-2 border-dark z-40 flex justify-around items-center py-2 px-2 shadow-brutal">
        ...
        <button 
          ...
          className={`flex-1 max-w-[80px] flex flex-col items-center gap-0.5 py-1 rounded-lg border transition-all duration-150 ${
            isDrawerOpen ? 'bg-secondary/20 border-dark' : 'border-transparent'
          }`}
        >
                ...
                return (
                  <Link key={item.path} href={item.path} onClick={() => setIsDrawerOpen(false)}>
                    <div className={`px-3 py-2 rounded-xl border-2 font-bold font-sans transition-all duration-150 cursor-pointer text-xs ${
                      isActive
                      ? 'bg-secondary border-dark shadow-brutal'
